...
const getById = (pIdParcela) => {
    return new Promise((resolve, reject) => {
        db.query('SELECT *, tamano_disponible - (SELECT sum(tamano) as suma FROM trozos WHERE fk_parcela = ?) as trozo_disponible FROM huerto.parcela WHERE id = ?', [pIdParcela, pIdParcela], (error, rows) => {
            if (error) reject(error);
